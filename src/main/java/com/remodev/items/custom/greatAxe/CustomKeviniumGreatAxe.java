...
public class CustomKeviniumGreatAxe extends SwordItem {
    private final Random random = new Random();

    public CustomKeviniumGreatAxe(Settings settings) {
        super(new CustomToolMaterial(), 6, -3.0f, settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip").formatted(Formatting.DARK_PURPLE));
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip2").formatted(Formatting.DARK_PURPLE));
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (!world.isClient) {
            world.playSound(null, target.getBlockPos(), ModSounds.MAGICATTACK, SoundCategory.PLAYERS, 0.2f, 2.2f);

            if (random.nextFloat() <= 0.15f) {
                target.addStatusEffect(new StatusEffectInstance(ModStatusEffects.CURSED_TOUCH, 100, 0));
            }
