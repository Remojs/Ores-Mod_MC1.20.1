...
public class CustomTominiteGreatAxe extends SwordItem {
    private final Random random = new Random();

    public CustomTominiteGreatAxe(Settings settings) {
        super(new CustomToolMaterial(), 3, -3.4f, settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip").formatted(Formatting.GREEN));
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip2").formatted(Formatting.GREEN));
