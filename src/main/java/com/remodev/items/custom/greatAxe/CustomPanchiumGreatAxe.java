...
public class CustomPanchiumGreatAxe extends SwordItem {
    private final Random random = new Random();
    private static final double EFFECT_PROBABILITY = 0.15;

    public CustomPanchiumGreatAxe(Settings settings) {
        super(new CustomToolMaterial(), 6, -3.0f, settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip").formatted(Formatting.GRAY));
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip2").formatted(Formatting.GRAY));
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (!world.isClient()) {
            if (random.nextDouble() < EFFECT_PROBABILITY) {
                target.addStatusEffect(new StatusEffectInstance(ModStatusEffects.ELECTROSHOCK, 100, 0));
            }
            if (random.nextDouble() < EFFECT_PROBABILITY) {
                target.addStatusEffect(new StatusEffectInstance(ModStatusEffects.ELECTROPARALYSIS, 120, 0));
            }
            world.playSound(null, target.getBlockPos(), ModSounds.ELECTRICEDGE, SoundCategory.PLAYERS, 0.15f, 1.4f);
        }
