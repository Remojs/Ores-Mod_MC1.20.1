...
public class CustomRobertiumGreatAxe extends SwordItem {
    private final Random random = new Random();

    public CustomRobertiumGreatAxe(Settings settings) {
        super(new CustomToolMaterial(), 7, -3.4f, settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip").formatted(Formatting.DARK_RED));
        tooltip.add(Text.translatable("item.pentacraft-ores.custom_greataxe.tooltip2").formatted(Formatting.DARK_RED));
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (!world.isClient) {
            world.playSound(null, target.getBlockPos(), ModSounds.FIREHIT, SoundCategory.PLAYERS, 0.4f, 1.3f);
        }
